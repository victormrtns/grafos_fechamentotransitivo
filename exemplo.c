...
for(i=0;i<20;++i){
    for(j=0;j<20;++j){
        for(k=0;k<20;++k){
            val = 0;
            if(((matrizadj[i][k] && caminho[k][j])) == 1 ){
                val = 1;
                break;
            }
        }
...
for(i=0;i<20;i++){
    for(j=0;j<20;j++){
       ...
       if(((caminho[i][j] && newprod[i][j])) == 0 ){
        caminho[i][j] = 0;
       }  
       caminho[i][j] = val;
    }
}
//Nesse for adjprod recebe newprod, pois ele vai ser usado para multiplicar com a matriz original no prod_bool e assim continua o ciclo
for(i=0;i<20;i++){
    for(j=0;j<20;j++){
